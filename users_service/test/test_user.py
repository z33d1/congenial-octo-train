...
class TestCardService(TestCase):
    ...
    def test_bad_request(self):
        res = self.client().get('/useras')
        self.assertEqual(res.status_code, 404)

    @patch("app.models.models.UserModel.get_user_info_by_id")
    def test_get_user(self, mock):
        mock.return_value = UserModel(id=2, username="smth_test")

        res = self.client().get("/user/2")

        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json, {"user_info": {
                                            "id": 2,
                                            "username": "smth_test"
                                        }
                                    })
